...
async function main() {
  ...
  try {
    ...
    assert(!as3.targetMonitor, "AS3 launch should select the host display automatically");
    ...
    assert(!spawnCalls[0].env.monitor, "AS3 spawn should not force a monitor model");
    ...
    assert(!as2.targetMonitor, "AS2 launch should select the host display automatically");
    ...
    assert(!spawnCalls[1].env.monitor, "AS2 spawn should not force a monitor model");
