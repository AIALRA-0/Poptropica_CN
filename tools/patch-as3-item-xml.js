...
function polishItemXml(content) {
...
function main() {
  ...
  const assetPatterns = parseList(args["asset-pattern"], ["game/data/items/"]);
  ...
  const rows = db
    ...
    .filter((row) => {
      ...
      return row.asset_type === "xml"
        && assetPath.includes("/game/data/items/")
  ...
  for (const assetRows of grouped.values()) {
    ...
    const translatedContent = polishItemXml(
  ...
  db.close();
  writeJson(path.join(paths.qaDir, "as3-item-xml-patch.json"), report);
  printJson(report);
}
