...
2026-06-23 09:38 EDT：修正 AS2 对话“同一句重复弹”和“上一句未关时下一句抢出”两类底层问题，并补上可复现截图/日志。根因是 `showSay()` 旧保护只拦同文本重复，若同一 NPC/脚本在气泡活动期间推入不同文本，旧逻辑会先 `hideSay(target)` 再显示新气泡；自然热区门控也只有 `1200/1600ms`，靠近 NPC 时容易连发。已修 `tools/lib/pack.js` 与定点运行补丁入口 `tools/patch-as2-gameplay-hud-popup.js`：活动气泡 2.8 秒内不允许被任何新 `showSay()` 抢占；自然对话全局门控提高到 2.8 秒，单目标门控提高到 3.5 秒；新增 QA 模式 `as2-show-say-active-throttle`。已重打 `gameplay.swf`/`gameplay-zh.swf` 和 AS2 runtime zip，补丁报告 `runtime-data/qa/as2/as2-gameplay-hud-popup-patch.json` 记录 frame_1 与 frame_9 均已替换。反向导出确认 `gameplay.swf` 含 `target.__zhLastShowSayAt < 2800`、`__zhDialogueGateUntil = _loc3_ + 2800`、`__zhLastDialogueAt < 3500`。G32QC/静音复测：活动抢句测试 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782221765583.json` 通过，OCR 为 `第一句对话测试`，server log 为 `QaShowSayActiveThrottle&suppressed=3&bubble=true`；同句重复回归 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782221840607.json` 通过，OCR 为 `重复对话测试`，server log 为 `QaShowSayThrottle&suppressed=4&bubble=true`。人工复核两张截图 `run-1782221765583/01-time-tangled-initial.png` 与 `run-1782221840607/01-time-tangled-initial.png`：均只显示一个中文气泡，角色可见，未出现连续抢泡。两轮均 `audioActive=0`、`visualGuardPassed=1`、`failedKeys=[]`。该项关闭 AS2 底层对话重入/抢句 blocker 的受控证据；自然 NPC 热区实走、时间装置路线和整岛封版仍待继续。
...
2026-06-23 03:04 EDT：修正并重验 Time Tangled `malidocs.swf` 任务文件关闭链。上一轮 popup-close 自动化存在点击坐标偏移：client 截图按钮中心约 `1123,104`，但脚本加了浏览器 chrome 偏移后实际点到 `1129,214`，导致点在纸板下沿而不是 `关闭`。已修 `tools/qa-as2-interaction-smoke.js`：client capture 直接使用 client 坐标；同时修 `tools/patch-as2-gameplay-hud-popup.js`，每次 HUD/popup 补丁后强制同步实际加载的 `gameplay-zh.swf`，并清掉 `zhOpenDirectMapBlockedByPopup` 旧短路分支。静态导出确认实际 `gameplay-zh.swf` 中只剩 `PopupClosePressed&target=openDirectMapBlockedMap` 新路径；G32QC/no-foreground/静音复测 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782198212806.json` 通过，`passed=1`、`popupClose.ok=true`、`failedKeys=[]`、`mapRequestCount=0`，点击日志为 `relativeX=1123,relativeY=104`，关闭日志为 `PopupClosePressed&target=mapMouseListenerBlockedMap`。人工复核截图 `runtime-data/qa/as2/interaction-smoke/run-1782198212806/01-time-tangled-popup-close.png`：弹窗关闭后回到 Mali 场景，角色可见，`关闭` 按钮无残留，没有误开地图。该项只关闭任务文件关闭链和 QA 坐标误判，不代表 Time Tangled 全岛封版；F11 真全屏安全输入、自然时间装置路线、自然 NPC no-repeat、外链白屏、全量音频仍未关闭。

2026-06-23 03:31 EDT：修正 AS2 HUD “看起来在右上但地图点不开”的验收缺口。失败样本 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782199199106.json` 显示 HUD 标注通过，但地图点击没有请求；新增 QA-only `MapMouseProbe` 后确认真实 Flash 坐标为 `x=759,y=-90`，旧热区为 `left=744,top=-38,right=820,bottom=50`，即 x 正确、y 被挡在热区上方。已修 `tools/patch-as2-gameplay-hud-popup.js` 和 `tools/lib/pack.js`：不移动右上 HUD 图像，只把隐藏地图按钮热区上边界扩到 `top=buttonY-100`、底部加 20px，并把探针限制为带 QA cache-bust 的测试启动。严格 G32QC/no-foreground/静音回归 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782199659821.json` 通过：`mapClicksPassed=1`、`hudAnchorPassed=1`、`visualGuardPassed=1`、`audioActive=0`、`failedKeys=[]`；日志同时出现 `popups/map.swf`、原生 `MapClicked` 和 `MapMouseProbe&x=759&y=-90&l=744&t=-138&r=820&b=70`。人工复核 `runtime-data/qa/as2/interaction-smoke/run-1782199659821/01-time-tangled-map.png`：地图面板实际打开，静态 `TIME TANGLED ISLAND` 美术字保留英文，`关闭` 按钮中文可见。随后复跑 `malidocs.swf` 关闭回归 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782199768333.json` 通过，截图 `runtime-data/qa/as2/interaction-smoke/run-1782199768333/01-time-tangled-popup-close.png` 显示弹窗已关闭、角色可见、无任务文件残留；但左侧仍可见深色 viewport 边，归入相机/边界 blocker，Time Tangled 仍未封版。

2026-06-23 04:32 EDT：恢复 AS2 普通 gameplay 视口后重新跑 Time Tangled 截图验收，修正上一轮把普通场景误裁到 640x480 的风险。最新 G32QC/no-foreground/静音严格回归 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782203179046.json` 通过：`ok=true`、`mapClicksPassed=1`、`hudAnchorPassed=1`、`visualGuardPassed=1`、`audioActive=0`、`failedKeys=[]`。人工复核 `runtime-data/qa/as2/interaction-smoke/run-1782203179046/01-time-tangled-initial.png`：角色可见，右上三枚 HUD 单行、不重叠，左上淡暂停按钮未见；`.../01-time-tangled-map.png`：AS2 地图真实打开，`关闭` 是中文按钮，静态 `TIME TANGLED ISLAND` 标题按规则保留英文原图。随后复跑 `malidocs.swf` 关闭链 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782203285075.json`，整体因深色边缘 visual guard 未通过而不计封版，但 `popupClose.ok=true`、`closeButtonStillVisible=false`、`mapOpenedDuringClose=false`，截图 `runtime-data/qa/as2/interaction-smoke/run-1782203285075/01-time-tangled-popup-close.png` 显示任务文件已关闭、角色可见、HUD 回到右上。当前结论：HUD/地图点击/任务文件关闭三个子项可留证通过；左侧深色 viewport 边仍是相机/边界 blocker，Time Tangled 继续重开中。

2026-06-23 06:31 EDT：修正 Time Tangled/Mali 下 AS2 map popup 被误套 640x480 tight viewport 的根因，并把地图弹窗验收从通用黑边 guard 拆成 map 专用 guard。根因是 `zhNotifyPopupViewport("map")` 的 direct map 标记被后续通用 popup 归一化覆盖成 boolean，导致地图被当成普通 640x480 弹窗放大裁切；`tools/patch-as2-gameplay-hud-popup.js` 已在最终归一化阶段强制恢复 `"map"`，导出 AS 复核确认 direct map 路径为 `zhNotifyPopupViewport("map")`。同时 `base.php`/`tools/lib/pack.js` 保留 `MAP_POPUP_VIEWPORT={x:0,y:0,width:1000,height:580}`，并新增 `tools/qa-helper.py analyze-map-popup-guard`，用地图纸张完整度、中心偏差和边缘蓝底替代普通场景黑边规则。最新 G32QC/no-foreground/静音回归 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782210583755.json` 通过：`ok=true`、`mapClicksPassed=1`、`sceneEvidencePassed=1`、`visualGuardPassed=1`、`playableCropGuardPassed=1`、`hudAnchorPassed=1`、`audioActive=0`、`failedKeys=[]`。人工复核 `runtime-data/qa/as2/interaction-smoke/run-1782210583755/01-time-tangled-initial.png`：角色可见、左上淡暂停按钮未见、右上 HUD 未裁切；`.../01-time-tangled-map.png`：地图纸张完整居中，`关闭`/`重启岛屿` 为中文按钮，静态 `TIME TANGLED ISLAND` 美术字保留英文。量化证据 `.../01-time-tangled-hud-anchor.json`：HUD `rightMargin=57`、`topMargin=1`、图标间距 `23/21`；地图 guard `.../01-time-tangled-map-visual-guard.json`：纸张 bbox `342,130-1103,724`、中心偏差 `0.005952`、边缘蓝底 top/left/right `0%`、bottom `1.683437%`。另新增 `EXTERNAL_REVIEW_PROMPT.zh-CN.md`，用于外部审计 HUD/viewport/popup 坐标体系。当前仍不能声明 Time Tangled 封版：F11 真全屏安全输入、自然时间装置路线、至少 3 段真实 NPC 对话 no-repeat、任务文件/物品使用后页面歪斜、外链白屏、地图介绍中文、全音频真实来源仍待继续验收。

2026-06-23 06:58 EDT：补上 Time Tangled Lab 对话链路的运行时证据，并修正此前 QA 对话 hook 没触发的根因。失败根因是 `tools/patch-as2-time-dialog-proof.js` 只在 URL fallback 中把 `time-lab` 映射到 `time-lab-help`，但当前 framework 通过 `_global.flashpointQaAs2Dialog` 传参时没有做同样归一化，导致 `manualSay` hook 静默返回。已修补并重打 `sceneLab.swf`；G32QC/no-foreground/静音复测三段原生 Time Lab 气泡均通过：`1782211802401` OCR 为“请帮帮我们。故障搅乱了时间，未来岌岌可危。”，`1782212034229` OCR 为“这份打印资料会说明我们需要你做什么。”，`1782212106889` OCR 为“那份打印件会解释一切。我们全靠你了。”；三轮均 `sceneEvidencePassed=1`、`visualGuardPassed=1`、`playableCropGuardPassed=1`、`hudAnchorPassed=1`、`audioActive=0`、`failedKeys=[]`，截图人工复核气泡位置正常、角色可见、没有中文贴图覆盖。另跑 AS2 底层 `showSay` 重复触发保护 `1782212191025`，OCR 为“重复对话测试”，server log 记录 `QaShowSayThrottle&suppressed=4&bubble=true&ticks=16`，即连续 5 次触发只保留 1 个气泡。该项关闭“Time Lab 受控三段原生中文气泡 + 底层同句重复节流”证据缺口；仍不等于自然 NPC/全岛路线封版，下一步还要走自然时间装置路线和真实热区对话。

2026-06-23 08:06 EDT：修正 Time Tangled AS2 地图 `重启岛屿` 确认链。根因是地图弹窗打开后，gameplay root 的全局地图鼠标监听仍会先收到后台点击；早期逻辑会误关地图，后续又只忽略点击，导致 `resetIslandButton.onRelease` 无法触发。已修 `tools/patch-as2-gameplay-hud-popup.js`、`tools/patch-as2-map-popup-text.js` 和 `tools/lib/pack.js`：弹窗状态不再误 `closePopup()`；map.swf 暴露原生 `showResetDialog()`；root 仅在 `__zhPopupMode=="map"` 且 root 坐标落入重启按钮区域时转发到原生确认框；确认框加防重复。G32QC/no-foreground/静音严格回归 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782216295331.json` 通过，`mapClicksPassed=1`、`mapResetConfirmPassed=1`、`sceneEvidencePassed=1`、`visualGuardPassed=1`、`playableCropGuardPassed=1`、`hudAnchorPassed=1`、`audioActive=0`、`failedKeys=[]`。人工复核 `runtime-data/qa/as2/interaction-smoke/run-1782216295331/01-time-tangled-map-reset.png`：地图纸张仍居中，静态 `TIME TANGLED ISLAND` 美术字保留英文，原生确认框居中，文案为“确定要重置时空缠结岛吗？该岛上的道具和进度都会丢失。”，按钮为“重置/取消”。日志记录 `MapResetRootBridge&x=38&y=314`。该项只关闭 Time Tangled 地图重启确认链和地图弹窗点击层级，不代表 Time Tangled 全岛封版；安全 F11 真全屏、自然时间装置路线、自然 NPC no-repeat、任务物品使用后布局、外链白屏、地图介绍和真实音频仍需继续。

2026-06-23 08:50 EDT：修正 AS2 最大化/F11 视觉验收自身的坐标和截图可信度问题，避免继续把截图污染或 letterbox 误判成游戏崩坏。`tools/lib/qa.js` 现在会把 `POPTROPICA_QA_MONITOR=G32QC` 注入 `capture-window`/`capture-window-sequence`，截图阶段不再退回旧窗口位置；`tools/qa-helper.py` 修正最大化后等待、避免 `bring_to_front()` 把已铺满显示器的窗口误 `SW_RESTORE` 回 1440x900，并让 `analyze-hud-diff` 按剔除黑色 letterbox 后的内容区计算 HUD 右边距；`tools/qa-as2-interaction-smoke.js` 的 F11 退出后默认等待从 1.2s 提到 6s，避免 Flash 短暂黑屏造成假失败。G32QC/静音回归：正常窗口 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782218866694.json` 通过，最大化可信截图模式 `as2-interaction-smoke-1782218776169.json` 通过，F11+恢复 `as2-interaction-smoke-1782218931728.json` 通过；三者均 `sceneEvidencePassed=1`、`visualGuardPassed=1`、`playableCropGuardPassed=1`、`hudAnchorPassed=1`、`audioActive=0`、`failedKeys=[]`。人工复核 `run-1782218776169/01-time-tangled-initial.png`、`run-1782218931728/01-time-tangled-f11.png`、`run-1782218931728/01-time-tangled-post-f11.png`：角色可见，HUD 位于内容区右上，边缘为黑色 letterbox 而不是 `#139ffd` 蓝底，静态美术字未硬盖中文。注意：最大化视觉截图在当前 Codex 窗口占用 G32QC 时必须短暂激活侧屏 Flashpoint 窗口；纯 no-foreground 最大化截图会捕到 Codex 背景，不能作为视觉验收证据。该项关闭 Time Tangled 代表样本的窗口/最大化/F11 视觉验收缺口，但不等于全岛封版。
...
| 窗口/F11/底部蓝边 | 未封闭 | 旧基线 `1782100652713` 与 wave 1 `1782150563288` 均通过 F11。修正右上锚点后新增 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782154039367.json`，截图 `run-1782154039367/01-time-tangled-f11.png` 显示 HUD 仍在右上、角色可见、未露明显蓝底。最新 `1782203179046` / `1782203285075` 证明 HUD/角色/地图/任务文件关闭正常，但人工复核仍可见左侧深色 viewport 边；因此相机/边界仍是 blocker，不能算窗口/边缘封闭 |
...
| HUD/菜单/地图 UI | HUD/地图点击通过；整岛仍开 | `1782153444792` 证明可见三枚 HUD 来自 gameplay 层；A/B 差分证据 `runtime-data/qa/as2/interaction-smoke/as2-hud-diff-1782154863479.json` 识别 3 个 HUD 图标，右边距 44px、上边距 0px、行内偏差 4px。最新严格回归 `runtime-data/qa/as2/interaction-smoke/as2-interaction-smoke-1782203179046.json` 通过，截图 `run-1782203179046/01-time-tangled-initial.png` 显示 HUD 单行右上、不重叠、无左上淡暂停按钮；`run-1782203179046/01-time-tangled-map.png` 显示地图真实打开，中文 `关闭` 按钮可见，静态地图标题保留英文原图。仍未覆盖所有菜单/背包/商店/确认框自然操作 |
...
| 当前剩余 | 封版重开 blocker | 不进入 Super Power。待清：左侧深色 viewport 边/相机边界、AS3 HUD 同口径复核、自然靠近 NPC 重复触发/抢话、任务物品/地图弹窗层级与黑残留/越界、使用物品后页面歪斜、科普/外链白屏、更多 Time 年代场景自然切换与时间机器物品链、真实音频来源清单。静态招牌/海报/美术字继续只记录或资产替换，不做中文叠层 |
...
17. Time Tangled 当前重开中：旧封版基线保留为历史证据；wave 1 已重新通过 loading 居中、Time Lab 中文对话、地图点击、F11、缺失请求和静音；wave 2 已量化关闭 AS2 Time 顶层 HUD 右锚和左上淡暂停残影；最新 `1782203179046` / `1782203285075` 证明 HUD、地图点击、任务文件关闭链仍可用，但左侧深色 viewport 边未封闭，AS3 HUD 同口径、对话节流、弹窗/外链 blocker 也未关闭。
